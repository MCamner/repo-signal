...
- `repo-signal brief [path] [--format text|markdown|json]` — compact daily
  health summary combining publish checklist score, risk map, and suggestions
  into a single fast scan. No API keys required.
- `brief.v1` JSON schema: `schema`, `repo`, `path`, `generated_at`,
  `last_commit`, `health` (score/total/label),
  `risks` (high/medium/low/total/top), `suggestions` (total/by_risk/top).
- `repo_signal/brief.py` — `build_brief()` and `format_brief()` with text,
